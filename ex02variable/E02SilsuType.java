...
public class E02SilsuType {
	...
	public static void main(String[] args) {
		...
		System.out.println("long1 + float1 ="+ result1); //결과:300.0
		
		/*
		float와 long의 연산 결과를 long으로 받고싶다면...
		방법1: float를 long으로 형변환 후 연산하거나...
		방법2: 계산결과 전체를 long으로 강제(명시적)형변환 한다. 
		 */
		long result2 = long1 + (long)float1;
		System.out.println("향변환후 연산="+ result2);
		...
		-소수점이 있는 데이터를 float에 대입하려면 접미사 f or F를 붙여야 한다. 
		-형변환도 가능하지만 Java에서는 권장하지 않는다. 
		 */
		float f3 = 100;
		//float f4 = 3.14; //에러발생. 3.14자체를 double형으로 인식한다. 
		float f4 = (float)3.14; //형변환 후 대입 가능하나 권장하지 않는다. 
		System.out.println("f4=" + f4);
		
		float f5 = 3.14F; //접미사를 통해 float형임으로 명시한다. (소문자f도 가능함)
		System.out.println("f4=" +f4);
		
		float f6 = f3 + f4;//float형 끼리의 연산은 기본규칙을 따른다.
		System.out.println("f6 =" + f6);
		
		double d1 = 3.14;
		double d2 = f6 + d1;//float + double => double 즉 큰쪽을 따라간다. 
