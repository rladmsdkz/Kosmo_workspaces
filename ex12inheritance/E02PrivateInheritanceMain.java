...
class A {}
class B extends A {}
=> 이경우 클래스 A, B에는 생성자를 정의하지 않았으므로 디폴트 생성자가
...
class Account {
	 ...
	 */
	public Account (int init) { //account 객체를 만들고 싶으면 이걸 통해서 생성을 해야함. 
		money = init;
	}
	
	//입금처리: protected 로 선언되었으므로 상속괸계에서 접근가능. 
	...
	//private 멤버변수를 외부로 반환할때 사용한다 (getter메서드)
...
class SavingAccount extends Account {
	...
	/*
	인자생성자에서 부모의 생성자를 호출하기 위핸 super()를 사용한다. 
	이때 인자가 하나인 부모클래스의 생성자가 호출된다. 
	현재 부모클래스에서는 인자가 하나인 생성자가 유일하므로 만약 아래와
...
//		sa.money = 100000; //에러발생
		...
		sa.showAccountMoney(); //잔액조희
		
		sa.saveMoney(-1000);//음수이므로 입금처리 안됨. 
		sa.showAccountMoney();
