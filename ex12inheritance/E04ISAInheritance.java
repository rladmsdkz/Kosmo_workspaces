...
//컴퓨터 : 기본적인 컴퓨터 환경을 제공하여 입력된 내용을 게산하다. 
class Computer {
	//소유주를 표현한 멤버벼수 
	String owner;
	...
	/*
	private 접근지정자로 선언된 멤버메서드는 외부 클래스 혹은 상속 받은
...
class NotebookComputer extends Computer {
	...
	batter = initCharge;
	super(name)
	아래는 부모의 인자생성자를 호출하고 있다. 
	 */
	public NotebookComputer (String name, int initCharge) {
		/*
		여기서 아래 super문장을 삭제하면 에러가 발생한다. 부모 클래스에서 
	...
	public void movigCal () {
		...
		if(battery<1) {
			...
			return; // 메서드에서 return은 종료를 의미한다.그래서 아래는 실행이 안된다. 
		}
...
/*
테블릿 노트북: 컴퓨터의 기본기능인 연산과, 휴대이용이 가능 및 펜으로 드로잉
	할 수 있는 기능까지 추가된 컴퓨터
 */
class TabletNotebook extends NotebookComputer {
	...
	public TabletNotebook (String name, int initCharge, String pen) {
		super(name, initCharge); //부모 클래스의 생성자 인자가 2개이기에. 
		registPencil = pen;
	}
