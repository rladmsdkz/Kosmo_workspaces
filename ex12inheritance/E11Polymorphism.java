...
class Child extends Parent {
	...
	//부모에서 정의한 메서드를 오버라이딩 하여 재정의한 멤버메서드
	@Override
	void parentMethod() {
		System.out.println("자식에서 Overriding한 메소드:parentMethod (int childMember");
	}
	//오버로딩해서 정의한 확장메서드
...
//		parent1.childMember = 1; //에러발생
		...
		/*
		잘못된방법
			-> (Child) parent2.childMember : parent2는 Parent 타입이므로 
								자식의 멤버변수인 childMember에 접근할 수 없다.
								따라서 에러가 발생하므로 형변환도 불가능하다.
								아래와 같이 Child타입으로 먼저 형변환 후 자식의
								멤버변수에 접근해야 한다. 
		 */
		...
		//자식타입으로 형변환한 이후에 해당 참조변수를 통해 자식멤버를 접근한다. 
		...
		/*
		Java에서 생성한 모든 클래스는 직/간접적으로 Object 클래스를 상속한다.
		따라서 모든 객체는 Object의 참조변수로 참조가 가능하다. 
		또한 Object클래스에서 정의된 모든 메서드를 별도의 선언 없이 사용할 수 있다. 
		 */
