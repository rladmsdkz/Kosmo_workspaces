...
이름만으로 접근이 가능하다.
단, 클래스 외부에서 접근할 경우 객체생성없이 클래스명을 통해서도 접근할 수있다. 
...
class Adder {
	...
	있으므로 클래스명을 통ㅇ한 접근이 허용된다.
	코드 레벨에서 보면 해당 클래스의 멤버변수이므로 변수명만으로 접근하는 것도 
	가능하다. 
	즉, 문법적으로는 둘다 허용된다. 
	 */
...
//부모의 멤버변수가 public이므로 접근의 제한이 없어 아래 코드는 모두 실행 가능하다.
class AdderFriend extends Adder {
	...
	public void showVal() {
		System.out.println("val="+ val); //변수명만으로 접근
		System.out.println("val="+Adder.val); //클래스명으로 접근
	}
}
public class E03StaticInheritance {
	...
	public static void main(String[] args) {
		...
		생성되어 메서드 영역에 로딩된다. 따라서 아레에서의 모든
