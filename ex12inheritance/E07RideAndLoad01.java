...
오버로딩: 메서드명은 같으나 매개변수의 갯수, 타입, 순서가 
	...
	하위클래서으세ㅓ는 오버로딩 된 모든 메서드가 포함된다. 

오버라이딩: 메서드명, 매개변수, 반환형까지 모두 동일한 
	메서드를 상속관계에 있는 하위클래스에서 재정의 한것을
	말한다. 
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		모든 영역에 접근할 수 있다. 따라서 아래 2개의 문장은
		모두 실행된다. 
		 */
...
//		ref1.loadmethod(1.1);//C클래스 영역은 접근불가 
		...
		자식으로 부모를ㄹ 참조하는것은 불가능하므로 아래 2개의 문장은 에러가 발생한다.
		 */
		
		A refId1 = new C();// A차입 -> C타입 : 참조가능
//		B refId2 = refId1;//B타입 -> A타입: 참조 불가능 (에러발생)
//		C refId3 = refId1;// C타입 -> A타입: 참조 불가능(에러발생)
		
		/*
		int a = 10.1; <= 에러발생
		double b = 10; //자동형변환
		int c = (int) 3.14; //강제형변환 후 대입
		=> 기본자료형의 경우 double이 int 보다는 더 큰 타입이므로 실수를 정수형변수에
		대입할 수 없다. 
		...
		있기 때문에 가능하고, 방식은 기본자료형과 동일하게 하면 된다. 
		*/
