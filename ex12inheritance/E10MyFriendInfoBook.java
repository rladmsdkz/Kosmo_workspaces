...
//고등학교 친구 정보를 저장할 클래스
class HighFriend extends Friend { //데이터 클래스
...
class FriendInfoHandler {
	...
	public FriendInfoHandler (int num) {
		...
		null로 초기화된다. 만약 정수탑의 배열이라면 0으로 초기화된다. 
		 */
		myFriends = new Friend[num];
		//최초 실행시 저장된 객체가 없으므로 0으로 초기화한다. 
		numOfFriends = 0;
	}
	//새로운 친구 연락처 추가 (1:고딩친구, 2:대딩친구 정보입력)
	public void addFriend (int choice) {
		...
		상속의 목적으로만 만들어졌기 때문이다. 아래의 공통정보는 해당 
		...
		3. numOfFriends를 1증가 시킨다. 후위증가이므로 배열에 먼저 저장된 후 
		증가된다.
		 */
		if(choice==1) {
		...
		/*
		각 객체를 생성 및 초기화한후 객체배열에 추가하면 Friend 타입으로 
	...
	1. 친구정보를 추가할때 High 혹은 Univ 객체를 배열에 저장한다.
	...
	public void searchInfo() {
		 ...
		 */
//		for(int i=0 ; i<numOfFriends.length ; i++) {
...
//			if(searchName.equals(myFriends[i].name)) {
	...
	public void deleteInfo() {
		...
		for (int i=0 ; i<numOfFriends ; i++) {
			...
			if(deleteName.compareTo(myFriends[i].name)==0) {
				/*
				 ...
				 것은 참조할 객체가 없다는 것이므로 이와같이 처리하면 된다. 
				 */
				myFriends[i] = null;
				//삭제된 원소의 index를 저장한다. 
				deleteIndex = i;
		...
		if(deleteIndex==-1) {
			...
			/*
			객체배열에서 검색된 원소를 삭제한 후 입력된 위치의 바로 뒤
			원소를 앞으로 하나씩 복사한다. 
			numOfFriend는 앞에서 1차감되므로 마지막 요소는 검색이나 출력에서 
			무시된다. 
			 */
...
public class E10MyFriendInfoBook { //기능을 가지고 있는 클래스
