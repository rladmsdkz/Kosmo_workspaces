...
-mian()메서드가 실행되기전에 이미 초기화되므로 main()메서드 실행시
...
class MyStatic {
	...
	int instanceVar;
	/*
	정적멤버변수: 객체를 생성할 필요 없이 클래스명으로 바로 접근할
		수 있다. 프로그램 시작시 이미 메서드 영역에 로딩된다. 
	 */
	...
	정적멤버메서드: 정적 메서드 안에서는 인스턴스형 멤버를 사용할
		...
		(※ main메소드에서는 static으로 선언된 메서드만 호출할 
		수 있었음을 기억하자.) 
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		System.out.println("myStatic.instancVar="+ myStatic.instanceVar);
		...
		/*
		하지만 정적 변수는 메서드 영역에 딱_하나만 생성되므로 어떤
		...
		myStatic.staticMethod(); //권장사항이 아니므로 클래스명을 사용하느것이 좋다. 
