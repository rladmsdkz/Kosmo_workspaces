...
-그 외는 메서드가 가진 모든 특성을 가지고 있다 (인수전달, 오버로딩 등) 
 */
public class PersonConstructor {
	...
	public PersonConstructor(String name, int age) {
		/*
		this()는 생성자에서 다른 생성자를 호출할때 사용할 수 없다. 
	...
	void initialize (String name, int age, String addr) {
		...
		멤버메서드는 개발자가 원할때 몇번이고 호출이 가능하므로 생성자와는
		특성이 다르다. 따라서 멤버메서드에서는 생성자를 호출할 수 없다. 
		 */
		
		//멤버변수 구분을 위핸 this는 멤버메서드에서도 사용할 수 있다. 
		this.name = name;
		this.age = age;
		this.addr = addr;
	}
