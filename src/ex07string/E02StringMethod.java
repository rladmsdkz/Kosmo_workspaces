...
public class E02StringMethod {
	...
	public static void main(String[] args) {
		...
		System.out.println("오늘은좋은날".length()); 
		System.out.println("==================================================");
		/*
		...
		/*
		3] codePointAt (인덱스)
			:문자열의 특정 인덱스의 문자를 찾은 후 아스키코드(유니코드)를 
			반환한다.
		 */
		...
		/*
		6] contains()
			: 문자열에 특정 문자열이 포함되어 있는지 판단하여 포함되었을때
			true를 반환한다. 
		 */
		...
		9] format()
			...
			printf와 사용법은 동일하고, 주로 웹애플리케이션을 제작할때
			사용한다. 
		 */
		//즉시 콘솔에 출력됨. 
		...
		//문자열을 서식에 맞춰 만든후에 반환한다. (ex.HTNL5) 이를 웹브라우저에 출력할 수 있다. 
		...
		//@가 문자열에 포함되어있는지 확인함 (-1이 아니면 포함된것을 의미)
		System.out.println(email1.indexOf("@")!= -1 ? "이메일형식맞음" : "이메일형식아님"); 
		
		
		/*
		시나리오] 주민등록번호를 indexOf 를 사용하여 성별을 구분하는
		프로그램을 작성하시오. 
		 */
		...
		System.out.println("java 를 KOSMO로 변경하기");
		...
		14] substring()
			...
			사용1:  substring(인덱스) => 이 경우 인덱스부터 끝까지 잘라낸다. 
