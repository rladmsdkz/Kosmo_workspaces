...
class Car {
		...
		public static final String MANUAL = "자동";
		...
		int carYear; //연식 : int형 
		Human owner; //소유주: Human 타입의 객체형 멤버변수
		
		//멤버메소드
		void drive () {
			
			/*
			owner는 개체이므로 이름을 출력하기 위해서는 "참조변수.name"과 같이 
			기술해야 한다. 
			 */
		...
		/*
		초기화메서드 1: 매개변수가 없는 형태로 항상 고정되 값으로만 
		...
		void initialize (String model, int year, String name, int age, int energy) {
			...
			carYear = year;
			/*
			또 다른 객체가 멤버변수로 정의되어있으므로, 객체를 생성한 후 각각의
			멤버변수를 초기화해야 한다. 
			 */
			owner = new Human ();
			owner.name = "캡틴로져스";
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		//ar1.drive(); <-에러발생
		/*
		실행시 예외가 발생한다. 자동차 소유주를 표현하는 Human 객체가 생성되지 
		않은 상태에서 출력을 시도하여 런타임 에러가 발생한 것이다. 
		 */
		
		System.out.println("[초기화메소드 호출후]");
		//초기화 메서드 호출을 통해서 Human()객체가 생성되고 초기화까지 진행된다. 
		car1.initialize(); // 여기를 호출함으로써 Human객체가 생성되게됨. 
		/*
		해당 초기화 메서드는 항상 고정된 값으로만 초기화되기 때문에 확장성이
		...
		/*
		자동차 2 객체생성: 객체생성 후 멤버변수에 직접 접근하여 초기화한다.
		...
		/*
		세번째 자동차 객체생성 : 메게변수가 있는 초기화 메서드를 사용하여 객체를
		초기화 한다. 이 경우 매개변수를 통해 다양한 값을 전달할 수 있으므로
		여러형태의 객체를 생성할 수 있다. 
