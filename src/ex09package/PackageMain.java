...
패키지 선언부: 현재 작성하는 클래스의 위치를 결정하기 위한 선언으로 
	PackageMain클래스는 ex09package 패키지에 생성된다. (묶는다)
 */

package ex09package;
/*
임포트 선언부: 현재 작성하는 클래스에 다른 클래스를 가져다 쓰기 위한 선언으로
	클래스가 생성된 패키지명과 클래스명을 함께 작성하면 된다. 
	만약 해당 패키지하위에 선언된 모든 클래스를 한번에 임포트하고 싶다면
	import java.util.*와 같이 와일드카드를 사용하면된다. 
 */
...
/*
import java.util.Date;
import ex09package.kosmo.util.CommonUtil; => 이와같이 임포트 해도된다. 
 */
