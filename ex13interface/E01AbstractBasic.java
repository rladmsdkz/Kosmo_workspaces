...
abstract class AbsClass1 {
	...
	//int형으로 표현할 수 있는 최대값을 반환하는 Wrapper 클래스의 상수
...
abstract class AbsClass2 {
	//추상메서드는 실행부가 없으므로 {}를 쓰지 않는다.
	abstract void absMethod (int number);
}

/*
위 클래스를 상속하면 부모가 가진 추상메서드를 그대로 상속받게 되므로 
해당 클래스도 abstract로 선언해야 한다. 단, 이 경우 해당 클래스로도 객체를
생성할 수 없게 된다. 만약 객체를 생성해야 한다면 추상메서드를 자식쪽에서 
오버라이딩 하면 된다. 
 */
...
//		AbsClass1 absClass1 = new AbsClass1();
		...
		//추상클래스는 객체(인스턴스) 생성은 할 수 없지만, 참조변수로는 사용할 수 있다. 
		...
		/*
		추상클래스내에 선언된 정적멤버메서드는 오버라이딩의 대상이 아니므로 
